...
void* fibonacci_over_vector(void *pos) 
{
    ...
    for (i = 0; i < stride; i++) 
    {
        //Obtengo la posición del vector que me toca calcular con mi hilo.
        int pos = i * nprocesadores + thread_id;
        //Calculo el fib de la posición obtenida anteriormente.
        vector[pos] = fibonacci(vector[pos]);
        //printf("Hilo: %d, pos vector: %d, fib: %d\n",thread_id,pos,vector[pos]);
